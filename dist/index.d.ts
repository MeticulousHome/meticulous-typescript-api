...
import { OSStatusResponse, APIError, AcknowledgeNotificationRequest, ActionResponse, ActionType, Actuators, BrightnessRequest, Communication, DeviceInfo, HistoryEntry, HistoryListingResponse, HistoryQueryParams, HistoryResponse, HistoryStats, LastProfileIdent, NotificationItem, ProfileIdent, ProfileShortIdent, ProfileUpdate, Settings, StatusData, Temperatures, WiFiConfig, WiFiCredentials, WiFiNetwork, WifiStatus, Regions, regionType, ManufacturingMenuItems, ManufacturingSettings, ShotRating, ShotRatingResponse, RateShotResponse, TestType, DefaultProfiles, CreateReportRequest, ReportRequestOptions, ReportErrorCode, DraftInfo, MeticulousIDRequestType, PaginatedResponse, PageParams, ReportPreflight, ReportInfo, SubmitInfo, UnlockMachineResponse } from './types';
...
export declare function getReportErrorCode(value: unknown): ReportErrorCode | undefined;
export default class Api {
    ...
    listProfiles(): Promise<AxiosResponse<Omit<Profile, 'stages'>[] | APIError>>;
    ...
    loadProfileFromJSON(data: Profile): Promise<AxiosResponse<ProfileShortIdent | APIError>>;
    loadProfileByID(id: string): Promise<AxiosResponse<ProfileShortIdent | APIError>>;
    ...
    getNotifications(acknowledged: boolean): Promise<AxiosResponse<NotificationItem[] | APIError>>;
     ...
     */
    unlockMachine(code: string): Promise<AxiosResponse<UnlockMachineResponse | APIError>>;
    setBrightness(brightness: BrightnessRequest): Promise<AxiosResponse<APIError | null>>;
    createReport(request?: CreateReportRequest, options?: ReportRequestOptions): Promise<ReportResult<DraftInfo>>;
    getReportPreflight(probes?: string[], options?: ReportRequestOptions): Promise<ReportResult<ReportPreflight>>;
    getReports(pageParams: PageParams, options?: ReportRequestOptions): Promise<ReportResult<PaginatedResponse<ReportInfo>>>;
    getDraftReport(localID: string, options?: ReportRequestOptions): Promise<ReportResult<Uint8Array>>;
    deleteDraftReport(localID: string, options?: ReportRequestOptions): Promise<ReportResult<void>>;
    updateReport(id: string, patch: Partial<ReportInfo>, options?: ReportRequestOptions): Promise<ReportResult<ReportInfo>>;
    markSubmittedReport(submitInfo: SubmitInfo, options?: ReportRequestOptions): Promise<ReportResult<void>>;
    getMeticulousReportTracking(serviceUrl: string, payload: MeticulousIDRequestType, options?: ReportRequestOptions): Promise<ReportResult<number>>;
